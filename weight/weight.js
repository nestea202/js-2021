...
function calculateAndShowWeights() {
    let weighthOnEarth = document.querySelector("#weight").value
    let mass = weighthOnEarth / surfaceGravity(EARTH)
    let weigthsOnPlanets = planets.map(planet => surfaceWeight(mass, planet))
    for (let index = 0; index < planets.length; index++) {
        let planet = planets[index];
        let weightOnPlanet = weigthsOnPlanets[index]
        //cream un nou rand 
